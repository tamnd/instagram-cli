...
describes what users get. Pick a format with `-o`, or let ig choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ig <command> -o table   # aligned columns for reading
ig <command> -o jsonl   # one JSON object per line, for piping
ig <command> -o json    # a single JSON array
ig <command> -o csv     # spreadsheet friendly
ig <command> -o tsv     # tab-separated
ig <command> -o url     # just the URL column
ig <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ig <command>            # a table, because this is a terminal
ig <command> | wc -l    # JSONL, because this is a pipe
```
